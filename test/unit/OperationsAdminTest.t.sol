...
contract OperationsAdminTest is DcaDappTest {
    ...
    function testSetPurchasePathAllowedRejectsEoa() external {
        bytes memory path = _dummyV3Path(address(1), address(2));
        vm.expectRevert(
            abi.encodeWithSelector(IOperationsAdmin.OperationsAdmin__EoaCannotBeHandler.selector, USER)
        );
        vm.prank(OWNER);
        operationsAdmin.setPurchasePathAllowed(USER, path, true);
    }

    function testSetPurchasePathAllowedRejectsMalformedPath() external {
        DummyDexPathHandler dummy = new DummyDexPathHandler(_dummyV3Path(address(1), address(2)));
        bytes memory bad = abi.encodePacked(address(1), uint24(3000)); // 23 bytes, not 20+n*23
        vm.expectRevert(
            abi.encodeWithSelector(IOperationsAdmin.OperationsAdmin__InvalidPurchasePath.selector, bad)
        );
        vm.prank(OWNER);
        operationsAdmin.setPurchasePathAllowed(address(dummy), bad, true);
    }

    function testSetPurchasePathAllowedRejectsEmptyPath() external {
        DummyDexPathHandler dummy = new DummyDexPathHandler(_dummyV3Path(address(1), address(2)));
        bytes memory empty;
        vm.expectRevert(
            abi.encodeWithSelector(IOperationsAdmin.OperationsAdmin__InvalidPurchasePath.selector, empty)
        );
        vm.prank(OWNER);
        operationsAdmin.setPurchasePathAllowed(address(dummy), empty, true);
    }

    function testSetPurchasePathAllowedRejectsMissingGetter() external {
        DummyERC165Contract dummy = new DummyERC165Contract();
        bytes memory path = _dummyV3Path(address(1), address(2));
        vm.expectRevert(
    ...
    function testSetPurchasePathAllowedRejectsRevertingGetter() external {
        DummyDexPathHandler dummy = new DummyDexPathHandler(_dummyV3Path(address(1), address(2)));
        dummy.setRevert(true);
        bytes memory path = _dummyV3Path(address(1), address(2));
        vm.expectRevert(
            abi.encodeWithSelector(IOperationsAdmin.OperationsAdmin__InvalidDexHandler.selector, address(dummy))
        );
    ...
    function testSetPurchasePathAllowedRejectsMalformedGetterReturn() external {
        DummyDexPathReturnsUint dummy = new DummyDexPathReturnsUint();
        bytes memory path = _dummyV3Path(address(1), address(2));
        vm.expectRevert(
            abi.encodeWithSelector(IOperationsAdmin.OperationsAdmin__InvalidDexHandler.selector, address(dummy))
        );
    ...
    function testSetPurchasePathAllowedRejectsNonCanonicalActivePath() external {
        DummyDexPathHandler dummy = new DummyDexPathHandler(hex"01");
        bytes memory path = _dummyV3Path(address(1), address(2));
        vm.expectRevert(
            abi.encodeWithSelector(IOperationsAdmin.OperationsAdmin__InvalidDexHandler.selector, address(dummy))
        );
    ...
    function testSetPurchasePathAllowedAndRevokeNonActive() external {
        bytes memory active = _dummyV3Path(address(1), address(2));
        bytes memory other = _dummyV3Path(address(3), address(4));
        DummyDexPathHandler dummy = new DummyDexPathHandler(active);

        vm.startPrank(OWNER);
        vm.expectEmit(true, false, false, true, address(operationsAdmin));
        emit OperationsAdmin__PurchasePathAllowedSet(address(dummy), keccak256(other), other, true);
        operationsAdmin.setPurchasePathAllowed(address(dummy), other, true);
        assertTrue(operationsAdmin.isPurchasePathAllowed(address(dummy), keccak256(other)));

        operationsAdmin.setPurchasePathAllowed(address(dummy), other, false);
        assertFalse(operationsAdmin.isPurchasePathAllowed(address(dummy), keccak256(other)));
        vm.stopPrank();
    }

    function testCannotRevokeActivePurchasePath() external {
        bytes memory active = _dummyV3Path(address(1), address(2));
        DummyDexPathHandler dummy = new DummyDexPathHandler(active);

        vm.startPrank(OWNER);
        operationsAdmin.setPurchasePathAllowed(address(dummy), active, true);
        vm.expectRevert(
            abi.encodeWithSelector(
        ...
        operationsAdmin.setPurchasePathAllowed(address(dummy), active, false);
        vm.stopPrank();
    }

    function testSetPurchasePathAllowedRejectsUnchanged() external {
        bytes memory active = _dummyV3Path(address(1), address(2));
        DummyDexPathHandler dummy = new DummyDexPathHandler(active);
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                IOperationsAdmin.OperationsAdmin__PurchasePathPermissionUnchanged.selector,
                address(dummy),
                keccak256(active),
                true
            )
    ...
    function testRevokeOnInvalidHandlerStillNamed() external {
        DummyERC165Contract dummy = new DummyERC165Contract();
        bytes memory path = _dummyV3Path(address(1), address(2));
        vm.expectRevert(
            abi.encodeWithSelector(IOperationsAdmin.OperationsAdmin__InvalidDexHandler.selector, address(dummy))
    ...
    function testRequirePurchasePathSetterRejectsNonSwapper() external {
        bytes memory path = _dummyV3Path(address(1), address(2));
        DummyDexPathHandler dummy = new DummyDexPathHandler(path);
        vm.prank(OWNER);
        operationsAdmin.setPurchasePathAllowed(address(dummy), path, true);
        ...
        operationsAdmin.requirePurchasePathSetter(USER, keccak256(path));
    }

    function testRequirePurchasePathSetterKeysByCallingHandler() external {
        bytes memory pathA = _dummyV3Path(address(1), address(2));
        bytes memory pathB = _dummyV3Path(address(3), address(4));
        DummyDexPathHandler handlerA = new DummyDexPathHandler(pathA);
        DummyDexPathHandler handlerB = new DummyDexPathHandler(pathB);

        vm.startPrank(OWNER);
        operationsAdmin.setPurchasePathAllowed(address(handlerA), pathA, true);
        operationsAdmin.setPurchasePathAllowed(address(handlerB), pathB, true);
        vm.stopPrank();
        ...
        vm.prank(address(handlerB));
        operationsAdmin.requirePurchasePathSetter(OWNER, keccak256(pathA));

        vm.prank(address(handlerA));
        operationsAdmin.requirePurchasePathSetter(OWNER, keccak256(pathA));
        vm.prank(address(handlerA));
        operationsAdmin.requirePurchasePathSetter(SWAPPER, keccak256(pathA));
    }

    function testOnlyOwnerCanSetPurchasePathAllowed() external {
        DummyDexPathHandler dummy = new DummyDexPathHandler(_dummyV3Path(address(1), address(2)));
        vm.expectRevert(ownableUnauthorized(USER));
        vm.prank(USER);
        operationsAdmin.setPurchasePathAllowed(address(dummy), _dummyV3Path(address(1), address(2)), true);
    }

    function _dummyV3Path(address tokenIn, address tokenOut) private pure returns (bytes memory) {
