...
Live deploy scripts still register Tropykus after this PR. Dropping that registration is PR 18; doing it here would break `make moc-tropykus`. The `tropykus-legacy/` name is the signal, not a deploy-script change.
...
- [ ] Idle handler (PR 12), LayerBank handler (PR 15), deploy/constants/harness/CI matrix (PR 18).
...
- Cutover: none. Frontend index map is unchanged until PR 18.
