...
PR 25, GitHub [#69](https://github.com/BitChillRSK/dca-contracts/pull/69). Stack on R13 (PR 24). Land before R9: R30 left only 329–452 bytes of EIP-170 margin on the two Dex handlers, and R9 must add share-transition emissions to the shared lending path. (**R53 re-baseline:** those margins were measured unoptimized — see [Measurement basis](./README.md#measurement-basis). Under the profile #104 pins the Dex leaves carry 8,884–9,097 B, so the ordering constraint and the conditional follow-up this spec attached to `supportsInterface` were answering a budget that no longer exists. The trim shipped and the ABI-coherence reason for it stands on its own.)
