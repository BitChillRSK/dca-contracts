...
- [ ] Add one internal slippage-settings validator covering the minimum ceiling, safety ceiling, and `minimum >= safety` relation.
- [ ] Use it in the constructor, `setAmountOutMinimumPercent`, and `setAmountOutMinimumSafetyCheck`.
- [ ] Reuse the existing custom errors unless a new error is necessary to make the failing relation unambiguous; document any error-ABI addition.
- [ ] Add a regression where the owner raises safety above the current minimum and state remains unchanged.
- [ ] Preserve event ordering and old/new values for successful updates.
...
- [ ] Constructor and both setters share one validation rule.
- [ ] No reachable state has `s_amountOutMinimumSafetyCheck > s_amountOutMinimumPercent`.
- [ ] Valid settings, events, swap behavior, and measured-cash accounting are unchanged.
- [ ] No selector or storage-layout change unless a narrowly justified error addition is recorded.
...
- ABI: no function/event change; possible custom-error addition only if the existing errors cannot describe the invariant.
- Scripts: none.
