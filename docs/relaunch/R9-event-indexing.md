...
- Scripts: none.
- Runtime (no IR, and pre-optimizer — see [Measurement basis](./README.md#measurement-basis)): `DcaManager` 22,698 (margin 1,878); `SovrynErc20HandlerDex` 22,958 (margin 1,618); `LayerBankErc20HandlerDex` 23,418 (margin 1,158). The new logs live on the shared lending/fee bases, so every lending handler grew; Dex remains under EIP-170.
