...
- ABI: **yes, deliberately.** One external view (`getUsersLendingTokenBalance` → `getUserShares`), two event names, one error name, and several event parameter names. New `topic0` for the two renamed events; parameter order and indexing unchanged. This is pre-freeze by design — R9 (PR 19) is the freeze, and the relaunch deploys fresh with no live log consumer, so the rename is free now and expensive afterwards.
