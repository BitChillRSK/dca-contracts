...
**Deploy scripts.** No change needed. `DeployMocSwaps`, `DeployDexSwaps`, `DeployIdleHandler`, `DeployLayerBankHandler`, and `DeployUsdrifHandler` each construct a fresh handler per assignment, so none of them can now revert on reuse. `VersionedRouteAccountingTest` deploys two LayerBank handlers through `DeployLayerBankHandler.deployMocksAndHandler` and proves the two routes keep independent shares, interest, and principal. Route v1 holds 400 DOC and route v2 holds 100, and each route's accrued interest stays bounded by its own deposit.

**This test is not a guard on the uniqueness check.** Verified by removing the check and re-running: all four cases still pass. That is inherent, not a defect in the test — R47 makes the shared-handler state unconstructible through `assignTokenHandler`, so no test driving the public API can reach the bug it prevents. The guards on the check are the `OperationsAdminTest` cases: with the check removed, `testHandlerAddressCannotBackTwoLendingRoutes`, `testHandlerAddressCannotBackTwoIdleRoutes`, `testHandlerAddressCannotBeReusedForASecondToken`, and `testHandlerAddressCannotBeReusedAcrossRouteClasses` all fail. The other two, `testDistinctHandlerInstancesRemainAssignable` and `testRevertedAssignmentDoesNotConsumeHandlerAddress`, pass with or without it by design — they guard the opposite direction, that the check is not over-broad and does not fire early.

What `VersionedRouteAccountingTest` covers is the remedy R47 forces on ops: when a fresh handler per pair is the only legal shape, that shape must still account correctly. The spec's scope bullet asked for exactly this ("two versioned routes with two distinct handlers preserve independent principal/share accounting"), and it is worth keeping on that basis, not as an R47 regression.
