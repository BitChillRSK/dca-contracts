...
| 4 | Exact-amount approvals go 0 → X → 0 every use (router per Dex batch; lending spender per deposit) | protocol / user | −23,010 exec, mostly refunded | **~−10,400 / use** (USDRIF) or **~−5,400** (DOC, USDT0), after a one-time ~20,000 at deploy (break-even ≈ 2–4 uses) | [R83](./R83-standing-spender-approvals.md) (answered: standing, both sites) |
