...
R22 deploy/CI (now PR 28) has not landed yet when this PR runs, so the LayerBank lane may still be add-on-only; `make check` / `make moc-tropykus` / both forks are the gate. If PR 28 somehow merges first, also run that CI lane.
