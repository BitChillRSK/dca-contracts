...
- Assigned: [R13-operations-admin-lifecycle.md](./R13-operations-admin-lifecycle.md) (PR 24). One owner-governance boundary, a typed form of the existing multi-swapper allowlist, an explicit one-shot route-class registry, and add-only versioned handler routes. **No open gates** — the migration gate was answered on 2026-08-26 as option (a), manual exit/re-entry, and idle became a route class rather than index `0`. Stack on R33.
...
- Next unassigned: `Start with R13` (PR 24). Stack on [#67](https://github.com/BitChillRSK/dca-contracts/pull/67). Both product decisions are recorded in the spec; ask nothing and implement. PR 2 (decision record) stays available when the human wants to record R18 / R19 / remaining optionals.
