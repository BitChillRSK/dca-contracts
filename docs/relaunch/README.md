...
- Next unassigned: `Start with R22 (deploy/CI)` (PR 16). No product gates. Stack on [#58](https://github.com/BitChillRSK/dca-contracts/pull/58). Handler-replacement in `OperationsAdmin` stays unassigned (later). PR 2 (decision record) stays available when the human wants to record R18 / R19 / optionals.
