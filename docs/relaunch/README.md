...
- Assigned: [R13-operations-admin-lifecycle.md](./R13-operations-admin-lifecycle.md) (PR 23). One owner-governance boundary, direct multi-swapper authorization, canonical protocol identity, and add-only versioned handler routes. Ask only the user-migration gate. Stack on [#66](https://github.com/BitChillRSK/dca-contracts/pull/66).
- Assigned: [R33-uniswap-slippage-validation.md](./R33-uniswap-slippage-validation.md) (PR 24). One constructor/setter slippage invariant. No product gate. Stack on R13.
- Assigned: [R31-handler-abi-trim.md](./R31-handler-abi-trim.md) (PR 25). Remove redundant handler selectors before R9; ask only whether fee-band mutation stays individual or becomes atomic-only. Stack on R33.
...
- Next unassigned: `Start with R13` (PR 23). Stack on [#66](https://github.com/BitChillRSK/dca-contracts/pull/66) and ask only the migration-policy gate in the spec. PR 2 (decision record) stays available when the human wants to record R18 / R19 / remaining optionals.
