...
- Assigned: [R31-handler-abi-trim.md](./R31-handler-abi-trim.md) (PR 25). Remove redundant handler selectors and dead abstract-base token parameters before R9; ask only whether fee-band mutation stays individual or becomes atomic-only. Also close R13's class↔handler ERC-165 hole (`ITokenLending` on `assignTokenHandler`), or assign a follow-up spec if Dex margin cannot absorb it. Stack on R13.
- Assigned: [R34-dca-manager-abi.md](./R34-dca-manager-abi.md) (PR 26). Canonical schedule reads and routing; ask the mutation-surface and consumer-cutover gates. Stack on R31.
...
- Next unassigned: `Start with R31` (PR 25). Stack on [#68](https://github.com/BitChillRSK/dca-contracts/pull/68). Ask only whether fee-band mutation stays individual or becomes atomic-only. Close the R13 class↔handler ERC-165 hole in the same PR (or assign a follow-up spec if Dex margin cannot absorb it). PR 2 (decision record) stays available when the human wants to record R18 / R19 / remaining optionals.
