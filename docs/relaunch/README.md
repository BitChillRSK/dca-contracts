...
- Assigned: [R31-handler-abi-trim.md](./R31-handler-abi-trim.md) (PR 25). Remove redundant handler selectors and dead abstract-base token parameters before R9; ask only whether fee-band mutation stays individual or becomes atomic-only. Stack on R13.
...
- Next unassigned: `Start with R31` (PR 25). Stack on [#68](https://github.com/BitChillRSK/dca-contracts/pull/68). Ask only whether fee-band mutation stays individual or becomes atomic-only. PR 2 (decision record) stays available when the human wants to record R18 / R19 / remaining optionals.
