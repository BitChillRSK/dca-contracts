...
- Next unassigned: `Start with R22 (deploy/CI)` (PR 17, [R22-deploy-ci.md](./R22-deploy-ci.md)). Constants, `DeployMocSwaps`, harness split, Makefile, and CI matrix for the new index map, plus the required LayerBank round-up solvency regression. No product gates. Stack on [#59](https://github.com/BitChillRSK/dca-contracts/pull/59). Handler-replacement in `OperationsAdmin` stays unassigned (later). PR 2 (decision record) stays available when the human wants to record R18 / R19 / optionals.
