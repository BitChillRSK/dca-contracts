...
- Assigned: [R13-operations-admin-lifecycle.md](./R13-operations-admin-lifecycle.md) (PR 24). One owner-governance boundary, a typed form of the existing multi-swapper allowlist, direct lending-route classification, and add-only versioned handler routes. Ask only the one-shot user-migration gate. Stack on R33.
...
- Next unassigned: `Start with R13` (PR 24). Stack on [#67](https://github.com/BitChillRSK/dca-contracts/pull/67). Ask only the one-shot user-migration gate. PR 2 (decision record) stays available when the human wants to record R18 / R19 / remaining optionals.
