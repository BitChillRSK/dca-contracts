...
- Assigned: [R22-layerbank-handler.md](./R22-layerbank-handler.md) (PR 15). LayerBank DOC + MoC handler at index 1. No product gates. Stacked on [#57](https://github.com/BitChillRSK/dca-contracts/pull/57).
- Next unassigned: `Start with R22 (deploy/CI)` (PR 16). No product gates. Stack on this PR once it is open. Handler-replacement in `OperationsAdmin` stays unassigned (later). PR 2 (decision record) stays available when the human wants to record R18 / R19 / optionals.
