...
- Next unassigned: `Start with R25` (PR 16, [R25-lending-redeem-naming.md](./R25-lending-redeem-naming.md)). Finish R16 leftover names (`_burn*` / `*ToRepay`). No product gates. Stack on [#58](https://github.com/BitChillRSK/dca-contracts/pull/58). Then `Start with R22 (deploy/CI)` (PR 17, [R22-deploy-ci.md](./R22-deploy-ci.md) — includes LayerBank round-up solvency regression). Handler-replacement in `OperationsAdmin` stays unassigned (later). PR 2 (decision record) stays available when the human wants to record R18 / R19 / optionals.
