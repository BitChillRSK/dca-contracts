...
- Assigned: [R29-hardcode-exchange-rate-scale.md](./R29-hardcode-exchange-rate-scale.md) (PR 20). Hardcode Sovryn/Tropykus `EXCHANGE_RATE_DECIMALS = 1e18` on the adapters (LayerBank already uses `RAY`). Stacked on [#63](https://github.com/BitChillRSK/dca-contracts/pull/63). Status URL pending PR open.
- Next unassigned: `Start with R22 (deploy/CI)` (PR 21, [R22-deploy-ci.md](./R22-deploy-ci.md)). Stack on R29. Handler-replacement in `OperationsAdmin` stays unassigned. PR 2 (decision record) stays available when the human wants to record R18 / R19 / optionals.
