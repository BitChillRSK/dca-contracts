...
- Planning: GitHub [#60](https://github.com/BitChillRSK/dca-contracts/pull/60). Specs only: R26 (PR 17), R27 (then PR 19), R28 (then optional late). Stacked on [#59](https://github.com/BitChillRSK/dca-contracts/pull/59). Later reordered so R27/R28 precede R22 deploy/CI — see [#62](https://github.com/BitChillRSK/dca-contracts/pull/62).
...
- Next unassigned: `Start with R22` (PR 29, deploy/CI). Stack on [#72](https://github.com/BitChillRSK/dca-contracts/pull/72). No product gates. PR 2 (decision record) stays available when the human wants to record R18 / R19 / remaining optionals.
- Later review queue: `DcaManager.setOperationsAdmin` lifecycle hardening and ownership-transfer hardening (`Ownable2Step` or equivalent acceptance flow) remain intentionally out of scope for PR 68 / R13 and should be scheduled as optional-late hardening work rather than forgotten.
