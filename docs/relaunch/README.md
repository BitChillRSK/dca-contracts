...
- Planning: GitHub [#60](https://github.com/BitChillRSK/dca-contracts/pull/60) (this PR). Specs only: R26 (PR 17), R27 (PR 19, Tropykus cash guards), R28 (optional late, `LendingErc20Handler`). Stacked on [#59](https://github.com/BitChillRSK/dca-contracts/pull/59).
- Next unassigned: `Start with R26` (PR 17, [R26-share-terminology.md](./R26-share-terminology.md)). Replace the non-standard “lending token” noun with `shares` (`getUserShares`, `_stablecoinToShares` / `_sharesToStablecoin`, `TokenLending__SharesRedeemed(Batch)`, `TokenLending__InsufficientShares`, Sovryn `_redeemShares`). Rename-only, ~370 refs. No product gates. Stack on [#60](https://github.com/BitChillRSK/dca-contracts/pull/60). Then `Start with R22 (deploy/CI)` (PR 18, [R22-deploy-ci.md](./R22-deploy-ci.md)) — constants, `DeployMocSwaps`, harness split, Makefile, CI matrix, plus the required LayerBank round-up solvency regression. Then `Start with R27` (PR 19, [R27-tropykus-lending-guards.md](./R27-tropykus-lending-guards.md)) — align Tropykus deposit/batch zero-cash guards with Sovryn/LayerBank; Tropykus is not in the new deploy map but the code must still be correct. Then R9 (PR 20). Optional late: [R28-lending-erc20-handler.md](./R28-lending-erc20-handler.md) extracts `LendingErc20Handler` (Idle out; cheapest before R9). Handler-replacement in `OperationsAdmin` stays unassigned (later). PR 2 (decision record) stays available when the human wants to record R18 / R19 / optionals.
