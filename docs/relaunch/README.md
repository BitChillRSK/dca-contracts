...
- Next unassigned: `Start with R25` (PR 16, [R25-lending-redeem-naming.md](./R25-lending-redeem-naming.md)). Rename-only redeem helpers / `*ToRedeem` locals. No product gates. Stack on [#58](https://github.com/BitChillRSK/dca-contracts/pull/58). Then `Start with R22 (deploy/CI)` (PR 17). Handler-replacement in `OperationsAdmin` stays unassigned (later). PR 2 (decision record) stays available when the human wants to record R18 / R19 / optionals.
