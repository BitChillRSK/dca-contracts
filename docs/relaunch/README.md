...
- Assigned: [R30-purchase-pipeline.md](./R30-purchase-pipeline.md) (PR 21, GitHub [#65](https://github.com/BitChillRSK/dca-contracts/pull/65); implementation pending). Combines review Candidates A+B: one shared purchase algorithm and one shared stablecoin-source seam; no ABI/product change. Stacked on [#64](https://github.com/BitChillRSK/dca-contracts/pull/64).
- Next unassigned: `Start with R22 (deploy/CI)` (now PR 22, [R22-deploy-ci.md](./R22-deploy-ci.md)). Stack on [#65](https://github.com/BitChillRSK/dca-contracts/pull/65). Review Candidates C–F and handler replacement in `OperationsAdmin` stay unassigned. PR 2 (decision record) stays available when the human wants to record R18 / R19 / optionals.
