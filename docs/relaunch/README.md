...
- Assigned: [R22-idle-handler.md](./R22-idle-handler.md) (PR 12, idle handler). No product gates. Stacked on [#52](https://github.com/BitChillRSK/dca-contracts/pull/52).
- Next unassigned: `Start with R22 (LayerBank)` (PR 13, LayerBank handler). No product gates. PR 2 (decision record) stays available when the human wants to record R18 / R19 / optionals.
