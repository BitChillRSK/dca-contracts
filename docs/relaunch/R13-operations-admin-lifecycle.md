...
`SovrynErc20HandlerDex` is a relaunch handler, so its 426 bytes are binding (Tropykus is legacy and is not redeployed). **R53 re-baseline (2026-09-02):** that measurement was unoptimized. Under the profile #104 pins, `SovrynErc20HandlerDex` is 15,479 B with 9,097 B of margin ([Measurement basis](./README.md#measurement-basis)), so reason 2 no longer holds — a source-side migration would fit. Reasons 1, 3, and 4 are not bytecode arguments and are unaffected. **Closed in [#111](https://github.com/BitChillRSK/dca-contracts/pull/111):** option (a) stands on those remaining reasons; there is no follow-up re-judgement PR and cooperative migration must not be reopened. (The paragraph below that still describes why even the old size budget rejected the feature is historical wording from before the optimizer correction.) A source-side migration needs redeem-all, share-balance zeroing, a registry-validated transfer to the destination, a destination-side pull that measures its own balance delta, plus errors, events, and `DcaManager` reconciliation of every affected schedule under `nonReentrant`. That does not fit. R31 is budgeted to recover headroom, not to fund a new feature.
