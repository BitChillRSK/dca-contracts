...
8. **A schedule is addressed by `scheduleId` alone, and its owner is a checked field** — `s_dcaSchedules` is `mapping(uint64 => DcaSchedule)`, so the key no longer carries `user` or `token` and neither can be inferred. Every external path that reaches a schedule must resolve it through `_ownedSchedule`, which reverts `DcaManager__InexistentSchedule` for an unknown id and `DcaManager__NotScheduleOwner` for somebody else's; the purchase path, which is `onlySwapper` and buys for the schedule's stored owner, instead checks existence and that the schedule's `token` matches the batch's. Do not add a schedule-touching entry point that skips either check, and do not reintroduce a `(user, token)` argument pair as a substitute — a caller-supplied owner is not a check. One omission is another account's funds, which is why `test/unit/ScheduleOwnershipTest.t.sol` walks the whole mutator surface rather than a sample of it. `s_scheduleIds[user][token]` is enumeration only: `getDcaSchedules` and the max-schedules bound read it, and no purchase ever does. See `docs/relaunch/R64-batch-calldata-and-schedule-keying.md`.
