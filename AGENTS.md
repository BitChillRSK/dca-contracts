...
- **Toolchain:** CI pins Foundry to `v1.7.1` in `.github/workflows/test.yml`. Match it locally (`foundryup -i v1.7.1`) before trusting the done-gate. Forge **1.8.0 is known-broken here**: 10 suites fail at `setUp()` with `vm.prank: cannot overwrite a prank until it is applied at least once`. That is a toolchain incompatibility this repo still owes a migration for, not a bug in your change — do not "fix" those tests to chase it, and do not unpin CI.
