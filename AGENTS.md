...
8. **A schedule is keyed by `(scheduleId, user)`, and that key is the ownership check** — `s_dcaSchedules` is `mapping(uint64 => mapping(address user => StoredSchedule))`. Reach a schedule through `_callersSchedule`, which is `s_dcaSchedules[scheduleId][msg.sender]`: a caller naming somebody else's id lands on an empty struct and is refused as an id they do not hold. Do not add an owner field to the stored struct and do not write an owner check — an entry point that compares a stored owner is reintroducing the bug the key exists to make impossible, and it would also cost a third slot. Never accept a `user` argument on a schedule mutator, for the same reason: a caller-supplied owner is not a check. The `onlySwapper` purchase path is the one place a schedule is reached for somebody else, so it takes the buyer from the batch and uses the same key, then checks that the schedule's `token` matches the batch's — a row of another stablecoin must not be debited by a handler that never holds its funds. Every getter is free to read any schedule. `StoredSchedule` is two slots and holds neither half of its key; getters return `DcaSchedule`, which is the stored fields plus the id, so a caller can act on whatever it reads. `s_scheduleIds[user][token]` is enumeration only: `getDcaSchedules` and the max-schedules bound read it, and no purchase ever does. See `docs/relaunch/R64-batch-calldata-and-schedule-keying.md`.

9. **A batch row carries no amount** — `Batch` names schedules and their buyers, and `_batchBuyRbtc` reads what each row spends from its schedule. Do not add a per-row amount back as a staleness guard: it was measured at ~450 gas per row, and what it bought was turning one user's mid-flight edit into a failed tick for every other row in the batch, in order to refuse a purchase at the amount that user had just asked for. `minRbtcOut` still binds on the rBTC the handler measures itself receiving, and a Uniswap route's oracle floor still scales with the actual input, so nothing about slippage rests on the caller's arithmetic. The `routeIndex` comparison is not the same thing and stays: no setter can change a schedule's route, so it is an integrity check that the row belongs to this batch's handler, not a guard against staleness.
